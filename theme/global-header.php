<nav class="navbar navbar-expand-lg global-nav navbar-bar-light bg-light sticky-top">
    <a class="navbar-brand" href="#">
        <img src="/suitntie/asset/image/logo/main-logo.png" width="125" alt="suitntie logo" />
    </a>
    <button class="navbar-toggler navbar-toggler-icon" type="button" data-toggle="collapse" data-target="#navbarNavAltMarkup" aria-controls="navbarNavAltMarkup" aria-expanded="false" aria-label="Toggle navigation">
        <div class="navbar-toggler-icon-x"><span></span><span></span><span></span></div>
    </button>
    <div class="navbar-collapse collapse navigation-list text-right" id="navbarNavAltMarkup">
        <ul class="navbar-nav ml-auto">
            <li class="nav-item global-nav-item">
                <a class="nav-link" href="/suitntie/index.php">首页</a>
            </li>
            <li class="nav-item global-nav-item">
                <a class="nav-link" href="/suitntie/tests/dimension-test.php">个人测试</a>
            </li>
            <li class="nav-item global-nav-item">
                <div class="dropdown">
                    <a class="dropdown-toggle nav-link" href="#" role="button" id="programLinks" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        专业探索
                    </a>
                    <div class="dropdown-menu pt-2 pb-2 text-right" aria-labelledby="programLinks">
                        <a class="dropdown-item" href="#">总览</a>
                        <a class="dropdown-item" href="#">Action</a>
                        <a class="dropdown-item" href="#">Another action</a>
                        <a class="dropdown-item" href="#">Something else here</a>
                    </div>
                </div>
            </li>
            <li class="nav-item global-nav-item">
                <a class="nav-link" href="">一对一科普</a>
            </li>
            <li class="nav-item global-nav-item">
                <a class="nav-link" href="">关于适途</a>
            </li>
            <li class="nav-item global-nav-item">
                <a class="nav-link" href="">成为导师</a>
            </li>
            <li class="nav-item global-nav-item userIcon">
                <div id="userInNav" class=""></div>
            </li>
        </ul>
    </div>
</nav>