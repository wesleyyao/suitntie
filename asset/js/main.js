$(document).ready(function () {
    ...
    $.get('/suitntie/public/api/index.php?title=' + title).done(function (data) {
        if (data) {
            ...
            //                 <i class="fas fa-exclamation-triangle"></i> 请先<a href="#/" data-toggle="modal" data-target="#userLoginModal">登录</a>，登录成功后才可开始答题。
            //             </div>
            //             <br/><br/>
            //         </div>
            //     </div>
    ...
    $('#testForm').submit(function (e) {
        ...
        if (numberOfAnswers === totalQuestions) {
            ...
            dimensions.forEach(function (item) {
                $('#dimension' + item.id).val(item.times);
            ...
            $.post('/suitntie/public/api/proceed-result.php', formData).done(function(data){
                ...
                console.log(result)
                if(result == 'success'){
                    window.location.replace('/suitntie/dimension-test-result.php');
                }
